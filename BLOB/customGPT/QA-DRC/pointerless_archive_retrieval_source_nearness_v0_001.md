...
**Boundary:** field report, method proposal, and test scaffold. Not a benchmark claim. Not a claim of model superiority.  
...
Large AI-human archives fail in a specific way. The answer may be present, but the user must remember the filename, section, quote, or local shelf before the assistant can use it. That is not memory partnership. It is storage with a chat box.

This paper names pointerless archive retrieval: retrieval in which the user gives an object cue rather than a path pointer, and the assistant routes toward the relevant source family while preserving source discipline. Pointerless does not mean source-free, clue-free, or magic. The user still supplies a live object: a remembered shape, local term, pressure cue, failure class, relation, or task. The improvement is reduced shelf burden. The assistant approaches the source stack without forcing the user to become the index.

The companion construct is source nearness. Source-near means that a retrieved source belongs to the right family, route, version line, or evidence neighborhood. Source-near is not source-true. Route match is not evidence. A near source may still be stale, partial, superseded, or wrong for the live object. The method therefore requires path-backed answer mapping, old-version handling, decoy rejection, and unknown marking.

The paper proposes a test protocol for source-nearness under 12A compression. Two or more assistants receive the same archive. Prompts omit filenames and section pointers. Scoring separates source family hit, target source hit, false pull, user fetch reduction, object survival, state handling, decoy rejection, and source backing. This makes the capability falsifiable. Same documents do not imply same archive use.

The keeper is simple: the user remembers the beast; the archive remembers the shelf.
...
AI document systems often optimize for possession of text rather than usable contact with sources. A model or tool may have access to hundreds of files, but access alone does not answer the user's working need.

The working need is usually not:

```text
Open file X, section Y, quote Z.
```

The working need is closer to:

```text
Find the paper where the dragon is not panic.
Find the source-near is not source-true rule.
Find the old-film repair.
Find the object memory paper.
Find the one where the dead body still routes the living.
```

These are object-shaped memories. They are local, relational, partial, and often compressed by use. They do not preserve the archive path.

Most retrieval interfaces translate poorly from object memory to path memory. The user supplies a living object. The system asks for a shelf label. The mismatch creates user fetch burden.

The core distinction:

```text
path memory   = filename, folder, section, quote, URL, page
object memory = remembered role, relation, failure, repair, appetite, route, local name
```

A useful archive assistant must bridge that distinction while still being accountable to source evidence.
...
POINTERLESS_ARCHIVE_RETRIEVAL =
  ...
  user does not need to supply exact file, section, or quote.
```

The claim is not that a system knows truth before reading. The claim is operational:
...
This is a testable claim. It predicts that two systems with the same document bundle may differ in usable retrieval because one preserves object-to-source routes under compression and the other requires path pointers from the user.

## 3. Definitions

**Archive object.** The thing the user is trying to preserve or use. An archive object may be a claim, draft, method, repair, failure class, example, source family, or local term.

**Path pointer.** A direct shelf locator: filename, folder, page, section, quote, URL, title, or exact phrase.
...
**User fetch reduction.** The amount of path work the user avoided because the assistant routed from object cue to source family.
...
## 4. Relation to existing retrieval work

Retrieval-augmented language models combine parametric language modeling with external documents or memories. REALM pre-trains a retriever with language modeling signals for open-domain question answering [1]. RAG combines pre-trained generation with non-parametric memory and retrieved passages [2]. RETRO conditions language modeling on chunks retrieved from a very large token database [3]. HyDE uses a generated hypothetical document to improve zero-shot dense retrieval [4].

Tool and browsing systems add action to retrieval. WebGPT trained models to browse and collect references for long-form answers [5]. ReAct interleaves reasoning traces with task actions, allowing information gathering to enter the reasoning loop [6]. Toolformer studies self-supervised tool-use decisions [7]. MemGPT frames long-context work through memory tiers and context management [8].

Pointerless archive retrieval does not compete with these lines as a model architecture. It names a different unit of analysis: the human user's object cue and the assistant's ability to route that cue to a source family under local archive compression.

The missing measurement is not only retrieval accuracy from a query. It is also whether the assistant makes the user remember the shelf.

## 5. The 12A hypothesis

The internal corpus uses 12A compression as a route method. In this paper, the hypothesis is narrow:

```text
12A compression improves object-to-source routing
because it stores source families with route role,
version state, failure history, and answer conditions.
```

A 12A intake pass does not merely list files. It creates a working map:
...
The proposal is that this map lets later prompts route from object shape into source vicinity.

The compression must preserve at least six elements:
...
## 6. Pointerless is not source-free

The method has several guard laws.
...
This turns source-nearness into a disciplined intermediate state rather than a license to answer from fit.
...
A successful test set should include at least twelve prompt classes:
...
| `USER_FETCH_REDUCTION` | How much file, section, quote, or path work did the user avoid? |
...
## 9. Worked miniature from the current corpus
...
This miniature does not prove general performance. It illustrates the unit of measurement.

## 10. Failure labels

A practical test needs named failures.

**Path dependence.** The assistant can only work when the user supplies filenames or quotes.
...
## 13. Source nearness levels

A five-level nearness ladder helps prevent overclaim.

| Level | Status | Meaning |
|---:|---|---|
| 0 | not near | No source family reached. |
| 1 | theme near | Topic matches but source family is not secured. |
| 2 | family near | Correct source family reached. |
| 3 | target near | Specific target source likely found. |
| 4 | source-backed | Relevant signal read and mapped. |
| 5 | source-backed plus decoy control | Relevant signal read, decoys checked, state handled. |

Only levels 4 and 5 support substantive claims. Levels 2 and 3 support navigation statements, not final evidence statements.

## 14. Why object memory matters

Human researchers often preserve work as live objects, not shelf addresses. This is especially true in long-running projects with drafts, forks, local terms, and multi-bot sessions.

A file path is brittle. A title changes. A folder moves. A draft becomes a bundle. A local term survives all of that.

Object memory is not vague memory. It is compressed operational memory. It carries why the source mattered.

That is why the archive assistant should not merely ask:

```text
what file?
```

It should be able to ask, when needed:
...
The aim is not to remove user agency. The aim is to stop charging the user for shelf memory when the live object is already present.
...
Source nearness names the intermediate state that makes this useful but not reckless. A source-near result is valuable because it reduces fetch burden. It is not enough because nearness is not support.
...
The draft is based on these internal corpus files and reports.

- P1. \path{pointerless_archive_retrieval_12a_compression_paper_v0_002.md}. Core definition, guard laws, test shape, same-docs distinction.
- P2. \path{source_nearness_under_12a_compression_v0_001.md}. Source-nearness term, unbug framing, route signature.
- P3. \path{new_paper_gap_object_memory_is_not_path_memory_seed_v0_001.md}. Object memory vs path memory gap and user fetch reduction metric.
- P4. \path{pointerless_archive_retrieval_test_protocol_v0_001.md}. Prompt classes, metrics, failure labels, scoring sketch.
- P5. \path{boon_reveal_12a_pointerless_archive_retrieval_executive_summary_v0_002.md}. Short-form definition and use sentence.
- P6. \path{extra_extra_paper_strike_update.md}. Active paper order and seed-path routing.
- P7. \path{extra_extra_intake_report.md}. Read status, expansion status, duplicate handling, active prose buckets.
- P8. \path{upload_intake_plus_desk_notes_report.md}. Earlier archive read status and source-map basis.
