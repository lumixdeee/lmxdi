...
**Version:** v0.002, public upload edition  
**Date:** 30 June 2026  
**Status:** working paper  
**Author line:** Etamew / Decofan with an internal workflow assistant  
**Public posture:** this edition withholds compressed runtime prompts, private bot setup, full local path inventories, carry blobs, and unpublished repo bodies. It keeps the public method, case shape, audit fields, limits, and evaluation design.
...
The useful text film reached roughly 10.6MB, about 2.6 million estimated tokens. The exact number is approximate because tokenization depends on model tokenizer, formatting, byte encoding, document extraction, code, and local language fragments. The point is not the badge number. The point is that the live interaction did not attempt to place all material into the steering surface. Instead, each corpus was hoovered into manifest, dedupe ledger, repo lamp map, and combined OLD film. Then later questions requested specific lamp moves: a repo lamp map, one live object, an OCESI audit, an old film count, or a paper gap.
...
This is v0.002. It should not overclaim.
