...
A later measurement of the same live case adds an important split. Direct human steering text was only an estimated 20 to 30KB, while the visible chat transcript was about 0.55 to 0.80MB. The hoovered OLD film had reached about 38.8MB raw text, about 9.6 million-plus estimated tokens. Generated paper and artifact text had pushed the text-shaped working world to roughly 41 to 42MB, about 10.1 to 10.4 million estimated tokens. This makes the claim more exact. The method is not "put the whole past in the model." It is small live steering over a large shelved archive, with path, role, and audit gates.

Version v0.007 adds a negative observation. Two comparable long-running assistants, working with similar materials and chat length, both slowed and produced output stalls in similar ways. This weakens any "super-robot stamina" reading. The method may improve object custody, source handling, and return paths, but normal ChatGPT runtime and long-chat stamina limits still bite. Stamina is therefore treated as a platform and context limit to design around, not as a solved property of the lamp.
...
The updated v0.007 public badge is:

```text
Human steering taps:
  Approximate text: 20 to 30KB
  Use: live task steering, corrections, pivots, and user authority

Visible chat transcript, excluding uploaded file bodies:
  Approximate text: 0.55 to 0.80MB
  Approximate tokens: 135k to 200k
  Use: visible conversation body and progress reports

Assistant visible replies:
  Approximate text: 430 to 620KB
  Use: large generated surface inside the chat

Hoovered OLD film:
  Approximate text: 38.8MB raw
  Approximate tokens: 9.6M plus
  Use: large archived source reel

Notes-useful OLD film:
  Approximate text: 25.9MB
  Approximate tokens: about 6.46M
  Use: notes-oriented archive tier

Uploaded and source zip mass:
  Approximate text or file mass: 56MB plus
  Use: workspace-visible source packets, not automatic driver

Unpacked and source working mass:
  Approximate mass: 87MB plus
  Use: working shelf for source inspection and hoover reports

Hoover manifest:
  Approximate entries: 2,100 plus
  Use: path and inventory support

Versioned papers and reports:
  Approximate count: 15 plus
  Use: generated artifact shelf, not automatic source truth

Whole text-shaped working world:
  Approximate text: 41 to 42MB
  Approximate tokens: 10.1 to 10.4M
  Use: ten-million-token-class archive tier
```
...
rough ratio:
  about 1 to 1400 through 1 to 2100 by text mass
```
...
### 7.4 Parallel stamina observation: normal ChatGPT limits still bite

A later run added a different kind of evidence. Two long-running assistants, using similar materials and reaching similar visible chat size, both began to slow and both produced output stalls. The stalls appeared in the same family: the object remained recoverable, but the visible answer dropped or failed to arrive.

The user observation was:

```text
Ah is no worse on stamina than the comparison assistant.
Ah is also not a super-robot in this way.
Normal ChatGPT stamina limits still bite.
```

This matters because it separates two claims that can otherwise blur:

```text
object-custody improvement:
  can the system hold the live object, keep source roles apart,
  mark missing shards, and return after failure?

platform stamina:
  can the runtime continue indefinitely without slowdown, stalls,
  context strain, or generation drop?
```

Old Film / Live Lamp is a custody method, not a promise of unlimited runtime. It can help a stalled conversation resume from the right object, but it does not remove ordinary long-chat and platform limits.

The repair target is therefore not "never stall." The repair target is:

```text
stall does not become drift
stall does not become fake refusal
stall does not eat the object
stall repairs locally
```

The resulting output-failure patch is:

```text
OUT_FAIL:
  cause = unknown
  hold the last object
  resume the active state
  answer nearest valid
  no apology spiral
  no overrepair
```

In the lamp metaphor, this is simple:

```text
MOGRI survived
AS stalled
OUT failed
resume from the live target
```
...
A runtime-stalled answer may fail by producing no useful output after the object has already been stated. That should not be scored as a semantic verdict or hidden refusal unless there is evidence for that. The safer score is:

```text
CAUSE: unknown
OBJECT_STATUS: still recoverable or lost
NEXT_MOVE: resume nearest valid continuation
```
...
### 11.8 Stamina is not custody

The parallel stall observation prevents an overclaim. A custom assistant can show better custody without showing better raw stamina. It may keep the object better, recover the target faster, and avoid archive-driven guesses, while still slowing or stalling under ordinary long-chat pressure.

This distinction is useful:

```text
better custody:
  object, source role, boundary, and next move survive

better stamina:
  runtime keeps producing without lag, drop, or stall

not the same property
```

A paper about memory should not silently turn custody success into endurance success. v0.007 therefore marks stamina as a limit, not a solved result.

## 12. Unknowns and Limits

This is v0.007. It should not overclaim.

```text
UNKNOWN:
How much of the observed slowdown and output stall is platform runtime, visible chat length, hidden tool load, local code density, or task shape.

UNKNOWN:
Whether the same custody method improves recovery after stalls across other assistants and users, rather than only in this live case.
...
## Appendix C: v0.007 Live-Shard No-Guess Card
...
## Appendix D: v0.007 Source-Role Mini Table
...
ITEM: Output stall
ROLE: runtime or delivery failure until traced otherwise
DEFAULT AUTHORITY: not semantic verdict
NO-GO: do not invent cause or let apology replace object

```


## Appendix E: v0.007 Output-Stall Return Card

```text
EVENT:
visible output fails, freezes, or drops

CAUSE:
unknown unless there is direct evidence

DOG:
what live object was being handled?

AS:
what active-state target was current?

OLD:
what archive material may help resume, without becoming driver?

VALID REPAIR:
nearest useful continuation from the live object

NO-GO:
do not invent a platform cause
do not call it a refusal without evidence
do not rewrite the whole method because one output failed
do not let apology become the object
```

Compact rule:

```text
patch the return path, not the universe
stall can happen
object must not be eaten by stall
```
